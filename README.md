...
- 📦 **在售商品**：查看在售商品列表，支持 Excel 导出
- 🚫 **批量取消活动**：智能批量管理商品活动，支持按活动名称分组操作
...
│   │   ├── products.css
│   │   └── products.js
│   └── cancel-activity/    # 取消活动模块
│       ├── cancel-activity.html
│       ├── cancel-activity.css
│       └── cancel-activity.js
├── js/                     # 核心脚本
│   ├── app.js              # 应用核心逻辑
...
- **权限控制**：模块级别的认证检查
- **并发控制**：智能管理批量操作的并发请求数量
- **状态管理**：支持复杂业务场景的状态追踪和恢复
...
### v2.2.0 (2025-09-14)
- 🚫 **新增批量取消活动模块**：支持多商品活动的智能批量管理
- 🎯 **智能分组机制**：按活动名称自动分组，提高操作效率
- ⚡ **并发控制优化**：限制并发请求数量，防止服务器过载
- 📊 **实时进度追踪**：批量操作可视化进度反馈
- 🔄 **模式切换功能**：单个/批量模式无缝切换
- 📁 **CSV导出支持**：操作结果可导出备份
- 🔧 **重置功能完善**：支持模块初始化和手动重置
